...
export async function POST(req: Request) {
  ...
  if (!payload.name || payload.name.trim().length < 3) return NextResponse.json({ error: 'invalid_name' }, { status: 400 })
  if (!payload.email || !EMAIL_RE.test(payload.email)) return NextResponse.json({ error: 'invalid_email' }, { status: 400 })
  if (!payload.idea || payload.idea.trim().length < 20) return NextResponse.json({ error: 'invalid_idea' }, { status: 400 })
  if (!payload.area) return NextResponse.json({ error: 'invalid_area' }, { status: 400 })
  if (!Array.isArray(payload.workTypes) || payload.workTypes.length === 0) return NextResponse.json({ error: 'invalid_workTypes' }, { status: 400 })
  if (!Array.isArray(payload.days) || payload.days.length === 0) return NextResponse.json({ error: 'invalid_days' }, { status: 400 })
  if (!Array.isArray(payload.timeSlots) || payload.timeSlots.length === 0) return NextResponse.json({ error: 'invalid_timeSlots' }, { status: 400 })
