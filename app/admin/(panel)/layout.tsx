...
export default function PanelLayout({ children }: { children: ReactNode }) {
  ...
  // Validación fina del token en cliente (el middleware ya bloqueó sin cookie).
  useEffect(() => {
  ...
  const mobileLinkClass = (active: boolean) =>
    `font-mono text-[12px] uppercase tracking-[0.12em] py-3 px-2 rounded-sm transition-colors block ${
      active ? 'text-sage-700 bg-cream-100' : 'text-ink hover:bg-cream-100'
    }`
          ...
          <nav className="hidden md:flex items-center gap-6">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.to}
                href={item.to}
                className={navLinkClass(isActivePath(pathname, item.to, item.end))}
              >
                {item.label}
              </Link>
            ))}
          ...
          <nav className="flex flex-col gap-1 px-4 pb-4">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.to}
                href={item.to}
                className={mobileLinkClass(isActivePath(pathname, item.to, item.end))}
              >
                {item.label}
              </Link>
            ))}
